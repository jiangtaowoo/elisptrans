...
class WordTranslateResult(object):
    __attrs__ = [
        ...
        'word_er', 'word_past', 'mean', 'sentence', 'basicmean', 'means', 'enmeans'
    ]
    ...
    def __init__(self):
        self.ada = ormadaptor.AdaptorORM('dictmodel.yaml', 'dictionary.db')
        self.word = ""         #单词本身
...
class BDTranslation(object):
    ...
    def _output_translate_result(result_info):
        print result_info.encode('utf-8')

    #API for single word translation
    @staticmethod
    def translate(word, sentence="", proxies=None):
        ...
        if result:
            BDTranslation._output_translate_result(result)

    #API for multiple words translation
    @staticmethod
    def batch_translate(words, sentence="", proxies=None):
        sess = requests.session()
        req_dict = dict()
        js_dict = dict()
        srclang = BDTranslation._detect_lang(word)
        ...
        for word in words:
            result = BDTranslation._translate_request(sess, req_dict, js_dict, word, sentence, srclang)
            if result:
                DBTranslation._output_translate_result(result)

if __name__=="__main__":
    if len(argv)==2:
        script, word = argv
        proxies = None
        BDTranslation.translate(word, sentence="", proxies=proxies)
    elif len(argv)==3:
        script, word, sentence = argv
        proxies = None
        BDTranslation.translate(word, sentence=sentence, proxies=proxies)
